...
SCT SCTBuilder(vector<node> vertices, bool debug) {
    ...
    for (int i = 0; i < nVertices; i++) {
        labelIndexMap[vertices[i].label] = i;
        T.label.push_back(i);
    ...
    while (!Q.empty()) {
        ...
        int p = (*curr).label[0]; 
        int pval = sortedIntersection((*curr).label, vertices[labelIndexMap[(*curr).label[0]]].neighbours).size();

        for (int v = 1; v < (*curr).label.size(); v++) {
            int val = sortedIntersection((*curr).label, vertices[labelIndexMap[(*curr).label[v]]].neighbours).size();
            if (val > pval) {
                pval = val;
                p = (*curr).label[v];
            };
        }
        //if (debug) {printVector(sortedIntersection((*curr).label, vertices[labelIndexMap[(*curr).label[p]]].neighbours));}
        shared_ptr<SCT> child = make_shared<SCT>(sortedIntersection((*curr).label, vertices[labelIndexMap[p]].neighbours), vector<shared_ptr<SCT>>(), vector<tuple<int, bool>>());
        (*curr).links.emplace_back(p, 1);
        (*curr).children.push_back(child);
        Q.push(move(child));

        vector<int> unionNeighbourhood = vertices[labelIndexMap[p]].neighbours;
        //if (debug) {
        //    cout << "pivot" << p << endl;
        //    printVector(unionNeighbourhood);
        //}
        ...
        for (int i = 0; i < diffSize; i++) {
            vector<int> neighbourhoodi = sortedIntersection((*curr).label, vertices[labelIndexMap[diff[i]]].neighbours);
...
int pivoter(vector<node> vertices, int k, bool debug) {
    shared_ptr<SCT> T = make_shared<SCT>(SCTBuilder(vertices, debug));
    int cnt = 0;
